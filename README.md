...
and replace `<A_COMMIT_HASH>` by the lastest commit hash (or your preferred revision):
    ...
    curl -fsSL https://raw.githubusercontent.com/rbekker87/aws-codebuild-extras/<A_COMMIT_HASH>/install >> extras.sh && . ./extras.sh
...
phases:
  install:
    commands:
      ...
      - curl -fsSL https://raw.githubusercontent.com/rbekker87/aws-codebuild-extras/<A_COMMIT_HASH>/install >> extras.sh
      - . ./extras.sh
```
|NAME|VALUE
|---|---|
